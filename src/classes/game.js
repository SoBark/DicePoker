// server-side game logic for a poker
...
//  this.deck = new Deck();
  ...
  this.startNewRound = () => {
    ...
    this.roundData.bets = [];
    this.NumRethrows = 0;
    this.ThrowDices(this.dicePerPlayer);
  ...
  this.moveOntoNextPlayer = () => {
    let handOver = false;
    console.log('Люди: ' + this.toString)
    console.log('Стадия:' + this.getStageName());
    if (this.getStageName() =='Rethrows'){
      console.log('Людей перебросивших:' + this.NumRethrows);
      ...
      if (!handOver && numNonFolds == 1) {
        ...
        handOver = true;
      } else if (this.NumRethrows == numNonFolds){
             handOver = true;
             this.rerender();
      ...
      if (this.isStageComplete()) {
        ...
        if (numNonFolds == 1) {
          ...
          if (this.roundData.bets.length == 1) {
            this.updateStage();
          } else { this.log('Error: Folded players should not be able to move.'); return console.error(); }
          // if (this.roundData.bets.length == 1) {
          //   handOver = true;
          //   const roundResults = this.evaluateWinners();
          //   for (playerResult of roundResults.playersData) {
        ...
        if (this.roundData.bets.length > 2) {return console.error();}
        //check if everyone folded except one player
        const [numNonFolds, nonFolderPlayer] = this.getNonFoldedPlayer();
        if (!handOver && numNonFolds == 1) {
          // everyone folded, start new round, give pot to player
  ...
  this.evaluateWinners = () => {
    ...
    for (let i = 0; i < this.players.length; i++) {
      if (this.players[i].getStatus() != 'Fold') {
        let h = Hand.solve(
          this.convertCardsFormat(this.players[i].dices)
        );
        handArray.push(h);
  ...
  this.ThrowDices = () => {
    for (let pn = 0; pn < this.getNumPlayers(); pn++) {
      this.players[pn].dices = [];
      this.players[pn].throwDices(this.dicePerPlayer);
    }

    this.refreshDices();
  };

  this.refreshDices = function () {
    for (let pn = 0; pn < this.getNumPlayers(); pn++) {
      this.players[pn].dices.sort((a, b) => {
        return a.compare(b);
      });
  ...
  this.rethrow = (socket, dices) => {
    //this.checkBigBlindWent(socket);
    const player = this.findPlayer(socket.id);
    //var dicesArray = dices.toString.split(' ');
    dices = dices.split(' ');
    dices = dices.filter(dice => dice!='')
    console.log(dices);
    for(let i = 0; i < dices.length; i++)
    {
      player.throwDice(dices[i] - 1);
    }
    this.NumRethrows++;
    this.moveOntoNextPlayer();
    return true;
