...
socket.on('dealt', function (data) {
  let i = 1
  $('#mycards').html(
    data.dices.map(function (c) {
      return renderCardButton(c, i++);
    })
  );
...
var rethrowDices = function() {
var checkedValue = ''; 
var inputElements = document.getElementsByName('rGroup');
for(var i=0; inputElements[i]; ++i){
      if(inputElements[i].checked){
           checkedValue = checkedValue + ' ' + inputElements[i].value;
      }
}
  socket.emit('moveMade', {move : 'rethrow', dices: checkedValue})
}
var bet = function () {
...
function renderCardButton(card, index) {
  //return ('<div class="dice_black"><input type="checkbox" name="rGroup" value="'+index+'" id="r'+index+'" checked="checked" /> <label class="whatever" for="r'+index+'"></label></div">')
  return (
    '<div class="dice_black" id="card"' +
    card.value +
    '" data-value="' +
    card.value +
    '">' + //'<input type="checkbox" id="dice'+index+'" name="dice'+index+'" value="'+index+'">' +
    card.value +
    ' ' +
    '</div>' + '<input type="checkbox" name="rGroup" value="'+index+'" id="r'+index+'"/><label class="whatever" for="r'+index+'"></label>'
  );
}
...
function renderOpponent(name, data) {
  ...
  {
    OpponentsDices = OpponentsDices + renderCard(data.dices[i]);
  }
  if (data.buyIns > 0) {
    if (data.text == 'Fold') {
      return (
        ...
        name +
        ' (Fold)</span><p><div class="center-align">'+OpponentsDices+'</div><br /><br /><br /><br /><br />' +
        data.blind +
        '<br />' +
        data.text +
      ...
      if (data.text == 'Their Turn') {
        if (data.isChecked)
          return (
            ...
            name +
            '<br />Check</span><p><div class="center-align">'+OpponentsDices+'</div><br /><br /><br /><br /><br />' +
            data.blind +
            '<br />' +
            data.text +
          ...
          return (
            '<div class="col s12 m2 opponentCard"><div class="card yellow darken-3"><div class="card-content black-text"><span class="card-title">' +
            name +
            '</span><p><div class="center-align">'+OpponentsDices+'</div><br /><br /><br /><br /><br />' +
            data.blind +
            '<br />' +
            data.text +
            '</p></div><div class="card-action yellow lighten-1 black-text center-align" style="font-size: 20px;">$' +
  ...
  if (data.buyIns > 0) {
    if (data.folded)
      return (
        ...
        ' | Bet: $' +
        bet +
        '</span><p><div class="center-align">'+OpponentsDices+'</div><br /><br /><br /><br /><br /><br /></p></div><div class="card-action green darken-3 white-text center-align" style="font-size: 20px;">$' +
        data.money +
        ' (' +
        data.buyIns +
        ' ' +
    ...
    if (data.folded)
      ...
      return (
        ...
        ' | Bet: $' +
        bet +
        '</span><p><div class="center-align"> ' +OpponentsDices+
        //renderOpponentCard(data.cards[0]) +
        //renderOpponentCard(data.cards[1]) +
        ' </div><br /><br /><br /><br /><br />' +
        data.endHand +
